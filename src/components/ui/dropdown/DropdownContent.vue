...
<template>
  <Transition name="dropdown">
    <div
      ...
      :style="
        props.side === 'right'
          ? { top: '0', left: `calc(100% + ${props.sideOffset}px)` }
          : { top: `calc(100% + ${props.sideOffset}px)` }
      "
      :class="
        cn(
          'absolute z-50 min-w-[8rem] overflow-visible rounded-xl border border-[var(--color-border-default)] bg-white py-2 shadow-[0_10px_30px_rgba(15,23,42,0.14)]',
          props.side === 'right' ? 'left-0' : props.align === 'end' ? 'right-0' : 'left-0',
          props.class,
        )
      "
    >
