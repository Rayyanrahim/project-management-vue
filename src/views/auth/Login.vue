<template>
  ...
  <div class="auth-panel">
    ...
    <form class="mt-4 space-y-3" @submit.prevent="onSubmit">
      <div>
        <Input
          ...
          :class="errors.email ? 'outline-red-400 focus:outline-red-400' : undefined"
          @blur="touch('email')"
      ...
      <div>
        <div class="relative">
          <Input
            ...
            :class="[
              'pr-10',
              errors.password ? 'outline-red-400 focus:outline-red-400' : undefined,
            ]"
            @blur="touch('password')"
    ...
    <p class="mt-3 text-center text-sm/6">
      <RouterLink :to="{ name: 'ForgotPassword' }" class="auth-link"> Forgot Password? </RouterLink>
    </p>
...
const onSubmit = async () => {
  ...
  try {
    const result = await authStore.login(validatedData.value)
